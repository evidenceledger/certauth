...
func HomeHandler(c goauth.Config) func(rw http.ResponseWriter, req *http.Request) {
	return func(rw http.ResponseWriter, req *http.Request) {
		...
		authcode1 := c.AuthCodeURL("some-random-state-foobar") + "&nonce=some-random-nonce"
		...
		<script type="text/javascript">
			...
			"https://cert.mycredential.eu/oauth2/auth?client_id=my-client&redirect_uri=https%3A%2F%2Fcert.mycredential.eu%2Fcallback&response_type=token%20id_token&scope=fosite%20openid&state=some-random-state-foobar&nonce=some-random-nonce",
			c.AuthCodeURL("some-random-state-foobar")+"&nonce=some-random-nonce",
			"/oauth2/auth?client_id=my-client&scope=fosite&response_type=123&redirect_uri=https://cert.mycredential.eu/callback",
		)))
	}
}
