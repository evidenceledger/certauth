...
func ProtectedEndpoint(c clientcredentials.Config) func(rw http.ResponseWriter, req *http.Request) {
	return func(rw http.ResponseWriter, req *http.Request) {
		theUrl := strings.Replace(c.TokenURL, "token", "introspect", -1)
		theValues := url.Values{
			"token": []string{req.URL.Query().Get("token")},
			"scope": []string{req.URL.Query().Get("scope")},
		}

		resp, err := c.Client(context.Background()).PostForm(theUrl, theValues)
		if err != nil {
